...
namespace CuaHang.Presentation
{
    public partial class fr_BC_HDN : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["A1:B1"].Value = "Siêu Thị";
        ...
        private void txtthongtin_TextChanged(object sender, EventArgs e)
        {
            if (op1.Checked)
            {
                string sql = @"SELECT * FROM dbo.tb_HDN INNER JOIN dbo.tb_CTHDN ON tb_CTHDN.mahdn = tb_HDN.mahdn WHERE dbo.tb_CTHDN.masp=N'"+txtthongtin.Text+"'";
                msds.DataSource = cn.taobang(sql);
            ...
            if (op2.Checked)
            {
                string sql = @"SELECT * FROM dbo.tb_HDN WHERE dbo.tb_HDN.ngaynhap=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
            ...
            if (op3.Checked)
            {
                string sql = @"SELECT * FROM dbo.tb_HDN WHERE dbo.tb_HDN.manv=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
        ...
        private void msds_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //Lấy thông tin các mặt hàng
            dong = e.RowIndex;
            sql = @"SELECT * FROM dbo.tb_HDN WHERE mahdn = '" + msds.Rows[dong].Cells[0].Value.ToString() + "'";
            tblThongtinHang = cn.taobang(sql);
            ...
            exRange.Range["B9:B9"].Value = "Tổng Tiền:";
            exRange.Range["C9:E9"].MergeCells = true;
            exRange.Range["C9:E9"].Value = tblThongtinHang.Rows[0][3].ToString();

            //Tạo dòng tiêu đề bảng
            sql = @"SELECT mahdn,masp,soluong,thanhtien FROM dbo.tb_CTHDN WHERE mahdn = '" + msds.Rows[dong].Cells[0].Value.ToString() + "'";
