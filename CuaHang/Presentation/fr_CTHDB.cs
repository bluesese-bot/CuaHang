using CuaHang.Business.Component;
...
namespace CuaHang.Presentation
{
    public partial class fr_CTHDB : Form
    {
        ...
        public void khoitaoluoi()
        {
            ...
            msds1.Columns[0].HeaderText = "Mã HDB";
        ...
        {
            string sql = "SELECT * FROM tb_CTHDB where mahdb = N'" + sohdb + "'";
        ...
        private void btnLuu1_Click(object sender, EventArgs e)
        {
            if (cbMatHang.Text != "")
            {
                ...
                string uptt = "update tb_HDB set tongtien=(SELECT sum(thanhtien) FROM tb_CTHDB WHERE dbo.tb_CTHDB.mahdb = dbo.tb_HDB.mahdb) where mahdb='" + txtMHD1.Text + "'";
        ...
        {
            cn.LoadTextBox(txtDongia, "select giaban FROM dbo.tb_Sanpham WHERE masp= N'" + cbMatHang.Text + "'");
            lbMatHang.Text = cn.LoadLable("SELECT tensp FROM dbo.tb_Sanpham WHERE masp=N'" + cbMatHang.Text + "'");
            txtTT.Text = (float.Parse(numSL.Text) * float.Parse(txtDongia.Text)).ToString();
